...
hist_folder = "hists_velocity_dispersion_sweep_for_recalc/"
hist_correct = hist_folder + "arg_" + ft_c + "_" + bt_c + "_" + r_c + "_" + rr_c + "_" + m_c + "_" + mr_c + "_correct.hist"
#oneD_names   = ['ft', 'm', 'mr']
...
# # # # # # # # # # # # # # # # # # # # # #
def match_hists(hist1, hist2, ver, pipe_name):
    print "matching histograms: "
    #using call here instead so the format of using it is on record
    call([" " + path + "nbody_test/bin/milkyway_nbody" + ver  
          + " -h " + path + "like_surface/" + hist1
          + " -S " + path + "like_surface/" + hist2 + " 2>>" + pipe_name], shell=True)
    print hist1, "\n", hist2
    print "\n"
    return 0


def oneD_recalc(sweep_name):
    for names in range(0, len(oneD_names)):
        para = oneD_names[names]
        value_file = hist_folder + "parameter_sweeps" +  name_of_sweeps + "/" + para + "_vals.txt"
        f = open(value_file, 'r')
        ...
        for line in f:
            hist_name = hist_folder + para + "_hists/" + "arg_"
            ss = line.split(' \n')
            name = (ss[0])
            ...
            hist_name += ft_tmp + "_" + bt_tmp + "_" + r_tmp + "_" + rr_tmp + "_" + m_tmp + "_" + mr_tmp + ".hist"
            pipe_name = hist_folder + "parameter_sweeps" +  sweep_name + "/" + para + ".txt"
            
            match_hists(hist_correct, hist_name, '', pipe_name)
        f.close()
        os.system("mv " + value_file + " 1D_like_surface/parameter_sweeps" + sweep_name)
        os.system("mv " + pipe_name + " 1D_like_surface/parameter_sweeps" + sweep_name)

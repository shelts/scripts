...
cmake_shared_flags = ["-DBOINC_RELEASE_NAMES=OFF", "-DSEPARATION=OFF", "-DCMAKE_FIND_ROOT_PATH_MODE_PROGRAM=NEVER", "-DNBODY_OPENMP=" + nbody_openmp_sep_opencl]

# CMake flags used for windows
cmake_static_flag = ["-DNBODY_STATIC=ON",  "-DBOINC_APPLICATION=ON",  "-DCMAKE_BUILD_TYPE=Release", "../milkywayathome_client"]

# Linux
if os == "linux":

    if arch == "64":
        execute(["cmake", "."] + cmake_shared_flags + cmake_static_flag)

...
# Make sure opsys and architecture are valid
opsys  = sys.argv[1]
arch = sys.argv[2]
nbody_openmp_sep_opencl = sys.argv[3]
assert opsys in ["win", "linux", "mac"], "ERROR: Unknown OS " + opsys
...
if opsys == "linux":
    ...
    if arch == "64":
        os.chdir("nbody_test")
        execute(["cmake", "../milkywayathome_client/", "-DCMAKE_FIND_ROOT_PATH=/srv/chroot/hardy_amd64", "-DOPENCL_LIBRARIES=/srv/chroot/hardy_amd64/usr/lib/libOpenCL.so", "-DOPENCL_INCLUDE_DIRS=/srv/chroot/hardy_amd64/usr/local/cuda/include/"] + cmake_shared_flags + cmake_static_flag)
